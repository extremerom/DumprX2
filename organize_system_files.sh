...
if [ "$SPV_COUNT" -gt 0 ]; then
    ...
    echo ""
    echo "   # Opción 1: Usando find (más portable)"
    echo "   find $SYSTEM_DIR -name \"*.spv\" -type f -exec git add {} +"
    echo "   git commit -m 'Add SPV shader binary files'"
    echo ""
    echo "   # Opción 2: Usando globbing (requiere bash con globstar)"
    echo "   shopt -s globstar nullglob"
...
echo "   # Opción 1: Usando find"
echo "   find $SYSTEM_DIR -path \"*/etc/*\" \( -name \"*.xml\" -o -name \"*.conf\" -o -name \"*.txt\" \) -type f -exec git add {} +"
